...
export function CashRegister() {
  const { todaySales, dailyEarnings, closeDailyCut, tables, currentUserRoleId } = useApp();
  ...
  const handleCloseCut = async () => {
    ...
    const inputVal = parseFloat(cashDifference);
    if (isNaN(inputVal)) {
    ...
    if (!isAdmin) {
      if (difference === 0) {
        toast.success('Caja cerrada. Todo en orden.');
      } else if (difference > 0) {
        toast.success(`Caja cerrada. Sobrante: $${difference.toFixed(2)}`);
      } else {
        toast.warning(`Caja cerrada. Faltante: $${Math.abs(difference).toFixed(2)}`);
      }
    } else {
  ...
  const visibleStatsCards = isAdmin ? statsCards : statsCards.filter(s => s.title === 'Productos Vendidos');
                  ...
                  <div className="text-xs text-zinc-400">
                    {sale.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between py-1">
                        ...
                        {isAdmin && <span className="text-green-400">${(item.price * item.quantity).toFixed(2)}</span>}
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <Label className="text-zinc-400">
                {isAdmin ? 'Diferencia de efectivo' : 'Efectivo en caja'}
              </Label>
              <Input
                type="number"
              ...
              <p className="text-xs text-zinc-500 mt-2">
                {isAdmin 
                  ...
                  : 'Ingresa la cantidad exacta de dinero físico que hay en la caja'
                }
              </p>
            </div>
